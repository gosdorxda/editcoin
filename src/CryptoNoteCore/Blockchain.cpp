...
bool Blockchain::is_tx_spendtime_unlocked(uint64_t unlock_time) {
  if (unlock_time < m_currency.maxBlockHeight()) {
    ...
    if (getBlockMajorVersionForHeight(static_cast<uint32_t>(m_blocks.size())) >= BLOCK_MAJOR_VERSION_4)
    {
        current_time += m_currency.lockedTxAllowedDeltaSecondsV2();
    }
    else
    {
        current_time += m_currency.lockedTxAllowedDeltaSeconds();
    }
...
bool Blockchain::checkCumulativeBlockSize(const Crypto::Hash& blockId, size_t cumulativeBlockSize, uint64_t height) {
  size_t maxBlockCumulativeSize = m_currency.maxBlockCumulativeSize(height, getBlockMajorVersionForHeight(height));
